...
TEST(DirectoryParser, IndexTheFile_OneFile) {
  ...
  // TODO(LINNI): Could check that the rowID is as expected.
  LLIter iter = CreateLLIter(offset_list);
  int* first;
  LLIterGetPayload(iter, (void**)&first);
  EXPECT_EQ(*first, 1);
  DestroyLLIter(iter);
...
// TODO(adrienne): add a check to make sure
// the fisrt and/or last movies aren't added twice.
...
TEST(DocumentSet, AddMovieToSet) {
  ...
  EXPECT_EQ(LookupInHashtable(mset->doc_index, doc_id, &result), 0);
  LinkedList lst = (LinkedList)result.value;
  LLIter iter = CreateLLIter(lst);
  int* cur;
  EXPECT_EQ(LLIterGetPayload(iter, (void**)&cur), 0);
  EXPECT_EQ(*cur, 15);
  DestroyLLIter(iter);

  EXPECT_EQ(LookupInHashtable(mset->doc_index, 75, &result), -1);
...
TEST(DocIdMap, Full) {
  ...
  // Some files
  // Not ideal; opportunity for improvement next time around.
  char *f1 = (char*)(malloc(sizeof('a')*10));
  strcpy(f1, "file1");
  PutFileInMap(f1, docs);
  EXPECT_EQ(NumElemsInHashtable(docs), 1);
  ...
  char *f2 = (char*)(malloc(sizeof('a')*10));
  strcpy(f2, "file2");
  PutFileInMap(f2, docs);
  EXPECT_EQ(NumElemsInHashtable(docs), 2);
...
TEST(FileCrawler, CrawlFilesToMap) {
  ...
  // TODO(LINNI): This assumes that the ids are unique, starting at 0.
  int ids[11] = {0};
...
TEST(MovieTitleIndex, Full) {
  ...
  char* title = (char*)(malloc(sizeof('a')*50));
  ...
  char* title2 = (char*)(malloc(sizeof('a')*50));
...
TEST(MovieTitleIndex, GetDocumentSet) {
  // TODO(LINNI): Implement this test.
  ...
  // TODO(LINNI): Check that the movies are all in the set
  HTKeyValue kvp;

  // Sleepless in Seattle
  set = GetDocumentSet(ind, "sleepless");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 1, &kvp), 0);

  set = GetDocumentSet(ind, "in");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 1, &kvp), 0);

  set = GetDocumentSet(ind, "seattle");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 1, &kvp), 0);

  // For the love of the Foo. In Foo not Foo.
  set = GetDocumentSet(ind, "for");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 42, &kvp), 0);
  
  set = GetDocumentSet(ind, "the");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 42, &kvp), 0);

  set = GetDocumentSet(ind, "love");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 42, &kvp), 0);

  set = GetDocumentSet(ind, "of");
  EXPECT_EQ(LookupInHashtable(set->doc_index, 42, &kvp), 0);
