...
const char* movie_row_A = "9.3|The Shawshank Redemption|R|"
  "Crime|142|Tim Robbins,Morgan Freeman,Bob Gunton";

const char* movie_row_B = "7.4|Back to the Future Part III|"
  "PG|Adventure|118|Michael J. Fox,Christopher Lloyd,Mary Steenburgen";

const char* movie_row_C = "5.6|AAAAAAAAA|R|Crime|120|"
  "Aaaaaa Aaaa,Bbbb Bbbbbb,Cccc Ccccc,Ddddd Dddd";

const char* movie_row_D = "2.3|XXXXXX|PG|Drama|100|";

const char* movie_row_E = "2.3||PG|Drama|100|Aaaa Aaa";
...
TEST(Movie, CreateManualAndDestroyMovie) {
  Movie* m1 = CreateMovie();
  m1->star_rating = 7.5;
  m1->content_rating = strdup("PG");
  m1->title = strdup("Sleepless in Seattle");
  m1->duration = 125;
  m1->genre = strdup("RomCom");
  m1->num_actors = 2;
  char** actors = (char**)malloc(10 * sizeof(char*));
  actors[0] = strdup("Tom Hanks");
  actors[1] = strdup("Meg Ryan");
...
char* MallocString(const char* str) {
  char* cr = (char*)malloc(sizeof('a') * strlen(str) + 1);
...
TEST(Movie, CreateFromAlternativeRow) {
  // Copying the string from a const to an array
  // (to be more similar to the actual use case)
  ...
  Movie* m1 = CreateMovieFromRow(row);
  ASSERT_EQ(5.6, m1->star_rating);
  ASSERT_EQ(0, strcmp(m1->content_rating, "R"));
  ASSERT_EQ(0, strcmp(m1->title, "AAAAAAAAA"));
  ASSERT_EQ(120, m1->duration);
  ASSERT_EQ(0, strcmp(m1->genre, "Crime"));
...
TEST(Movie, CreateFromMissingActorRow) {
  // Copying the string from a const to an array
  // (to be more similar to the actual use case)
  printf("============DONE BY LINNI CAI============\n");
  char row[1000];
...
TEST(Movie, CreateFromMissingTitleRow) {
  // Copying the string from a const to an array
  // (to be more similar to the actual use case)
  printf("============DONE BY LINNI CAI============\n");
  char row[1000];
